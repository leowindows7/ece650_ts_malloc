...
void check_merge(block *block_toFree) {
  ...
  if (block_toFree->prev != NULL) {
    block *block_toFreePre_ass =
        (block *)((char *)block_toFree->prev + block_toFree->prev->size +
                  sizeof(block));
  ...
  if (curr_block == NULL) {
    ...
    while (curr_block != NULL) {
      if ((curr_block->size >= size && curr_block->size - size < diff)) {
        ...
        if (diff == 0) { // if the space match exactly the same space needed, we can return directly 
    ...
    if (smallestDiff != NULL) { 
      return split_block(smallestDiff, size); // if there is no space that match exactly the same required space, return here
    }
    data_size += size + sizeof(block);
    block *new_block = sbrk(size + sizeof(block));  // no available space on the list, create a new one
    new_block->size = size;
    new_block->prev = NULL;
    new_block->next = NULL;
    head_block = new_block;
